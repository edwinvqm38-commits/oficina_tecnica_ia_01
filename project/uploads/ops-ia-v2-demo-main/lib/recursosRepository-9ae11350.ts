import { demoData, type Recurso, type ResourceFileMeta } from "@/lib/demoData";
...
function fileReference(file: ResourceFileMeta | null | undefined): string | null {
  if (!file) return null;
  return normalizeString(file.futureDriveUrl || file.futureDriveFileId || file.name) || null;
}
...
function mapSupabaseRecurso(row: SupabaseRecurso): Recurso {
  ...
  const fichaTecnica = fileMetaFromName(metadata.ficha_tecnica);
  const imagen = fileMetaFromName(metadata.imagen);
  const archivos = Array.isArray(metadata.archivos)
  ...
  return {
    ...
    resourceFiles: {
      fichaTecnica,
      imagen,
      fichasTecnicas: fichaTecnica ? [fichaTecnica] : [],
