...
import { FieldLockButton } from "@/components/ui/FieldLockButton";
import { DateTextInput } from "@/components/ui/DateTextInput";
import { QuotationCashflowDashboardView } from "@/components/quotations/QuotationCashflowDashboardView";
import { QuotationCashflowDrilldownPanel } from "@/components/quotations/QuotationCashflowDrilldownPanel";
import { QuotationMonthlyCashflowView } from "@/components/quotations/QuotationMonthlyCashflowView";
import { TechnicalProposalWorkspaceModal } from "@/components/quotations/TechnicalProposalWorkspaceModal";
import type { Cotizacion, DetalleRequerimientoItem, Recurso, Requerimiento } from "@/lib/demoData";
import { computeQuotationEconomicRows } from "@/lib/quotationEconomics";
import { collectQuotationCashflowItems, computeQuotationCashflow, computeQuotationCashflowWeeklyDrilldown, type QuotationCashflowData } from "@/lib/quotationCashflow";
...
type QuotationWorkspaceModalProps = {
  ...
  onDeleteRequirement?: (requirementId: string) => boolean | void;
...
export function QuotationWorkspaceModal({
  ...
  function handleDeleteRequirement(requirementId: string, codigo: string) {
    setPendingConfirm({
      message: `¿Deseas eliminar el requerimiento ${codigo}?`,
                            ...
                            <tr key={rq.id} className="h-[26px] border-t border-border">
                              ...
                              <td className="h-[26px] px-1 py-0 text-center">
                                ...
                                <button
                                  ...
                                  className="inline-flex h-5 w-5 items-center justify-center rounded border border-stone-200 text-[11px] text-stone-500 hover:border-rose-300 hover:bg-rose-50 hover:text-rose-700"
                                  title={`Eliminar ${rq.codigo}`}
                                  aria-label={`Eliminar ${rq.codigo}`}
                                >
                                  X
                                </button>
                                ) : null}
