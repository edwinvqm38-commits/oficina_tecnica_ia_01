...
import { DataTable, type DataTableViewState } from "@/components/DataTable";
import { QuotationWorkspaceModal } from "@/components/quotations/QuotationWorkspaceModal";
import { RequirementWorkspaceModal } from "@/components/RequirementWorkspaceModal";
import type { EditableRequirementItem } from "@/components/RequirementItemsGrid";
...
import { normalizeCotizacionEconomicSummary } from "@/lib/quotationEconomics";
import { formatCurrencyNumber, formatDate, normalizeDateForStorage } from "@/lib/utils";
import { publishDataSourceSnapshot, debugDataSourceLoad, type AppDataSource } from "@/lib/dataSourceDiagnostics";
import {
...
import { createRequirementFromWonQuotationSupabase } from "@/lib/requirementsRepository";
import { createCotizacion, CreateCotizacionError, updateCotizacion, UpdateCotizacionError } from "@/lib/quotationsRepository";
import {
  debugUiState,
...
export default function CotizacionesPage() {
  ...
  function saveRequirementOverlay() {
    if (!requirementDraft || !selectedRequirementId) return;
    if (isSupabaseReadOnly) {
      setWarning("Los requerimientos importados desde Supabase están disponibles solo en lectura por ahora.");
      return;
    }
    ...
    const normalized: DetalleRequerimientoItem[] = requirementItems
  ...
  function deleteRequirementFromQuotation(requirementId: string) {
    if (isSupabaseReadOnly) {
      setRequirementCreationError("La eliminación de requerimientos queda deshabilitada mientras la vista use Supabase en modo solo lectura.");
      return false;
    }
  ...
  return (
    <section className="app-table-section min-w-0">
      ...
      <QuotationWorkspaceModal
        ...
        onDeleteRequirement={
          !isSupabaseReadOnly && canOpenRequirementsFromQuotation && canViewQuotationActions
