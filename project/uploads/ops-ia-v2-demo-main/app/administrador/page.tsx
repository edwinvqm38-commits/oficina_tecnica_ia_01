...
import { useAuth, type AppRole, type AppStatus } from "@/components/auth/AuthContext";
import {
  getModulePermissions,
  listModulePermissionEmails,
  upsertModulePermissions,
  type ModulePermissions,
} from "@/lib/modulePermissionsRepository";
import {
  MODULE_PERMISSION_CATALOG_BY_KEY,
  MODULE_PERMISSION_OPTIONS,
  type PermissionModuleKey,
  type StandardPermissionKey,
} from "@/lib/modulePermissionsCatalog";
...
export default function AdministradorPage() {
  ...
  const updateAccessRequestsByProfile = useCallback(async (profile: AdminUserProfile, payload: Record<string, unknown>) => {
    if (profile.id) {
      await supabase.from("user_access_requests").update(payload).eq("user_id", profile.id);
    }

    if (profile.email) {
      await supabase.from("user_access_requests").update(payload).eq("email", profile.email);
    }
  }, []);

  const updateProfileStatus = useCallback(
    async (profile: AdminUserProfile, payload: Record<string, unknown>, successMessage: string) => {
      setActionLoadingId(profile.id);
      setError("");
      setMessage("");

      const { error: updateError } = await supabase.from("user_profiles").update(payload).eq("id", profile.id);

      if (updateError) {
        setActionLoadingId(null);
        setError(`No se pudo actualizar ${profile.email ?? "usuario"}. Revisa RLS o permisos de administrador.`);
        return;
      }
  ...
  const approveProfile = useCallback(
    async (profile: AdminUserProfile) => {
      const now = new Date().toISOString();
      const roleToApply = selectedRole[profile.id] ?? profile.role ?? "consulta";
      await updateProfileStatus(
        ...
        `Usuario ${profile.email ?? ""} aprobado como ${roleToApply}.`,
      );
      await updateAccessRequestsByProfile(profile, {
        status: "approved",
        reviewed_by: user.id,
        reviewed_at: now,
        updated_at: now,
  ...
  const rejectProfile = useCallback(
    async (profile: AdminUserProfile) => {
      ...
      await updateProfileStatus(
        ...
        `Usuario ${profile.email ?? ""} rechazado.`,
      );
      await updateAccessRequestsByProfile(profile, {
        status: "rejected",
        reviewed_by: user.id,
  ...
  const suspendProfile = useCallback(
    async (profile: AdminUserProfile) => {
      ...
      await updateProfileStatus(
        ...
        `Usuario ${profile.email ?? ""} suspendido.`,
      );
    },
    [updateProfileStatus],
  );
  ...
  const changeRole = useCallback(
    async (profile: AdminUserProfile) => {
      ...
      await updateProfileStatus(
        ...
        `Rol de ${profile.email ?? "usuario"} actualizado a ${roleToApply}.`,
      );
    },
    [selectedRole, updateProfileStatus],
  );

  const visibleProfiles = useMemo(() => profiles.filter((profile) => shouldShowForFilter(profile, filter)), [filter, profiles]);
